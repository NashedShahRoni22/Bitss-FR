...
import { Check, ShoppingCart, Loader2, ArrowLeft, Package, Tag } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        const response = await fetch(`https://paymentapi.bfinit.com/api/v1/products/product/show/${id}`);
        ...
        if (result.success && result.data) {
          ...
          if (result.data.subscription_periods && result.data.subscription_periods.length > 0) {
          ...
          <div className="rounded-xl bg-white p-8 shadow-md">
            ...
            {/* Combo Products if available */}
            {
               product.combo_products.length > 0 && 
                <div className="mt-8">
                  <h2 className="text-xl font-semibold text-gray-900">Included in this Combo</h2> 
            ...
            {product.category?.sort_description && (
              <div className="mt-8 rounded-lg bg-blue-50 p-4">
                <p className="text-sm text-blue-900">{product.category.sort_description}</p>
              </div>
            )}
                ...
                <div className="mt-6 space-y-3">
                  {product.subscription_periods.map((period) => {
                    ...
                    if (period.discount_type === 'percent') {
                      discountedPrice = totalPrice - (totalPrice * period.amount / 100);
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <p className="font-semibold text-gray-900">
                              {yearCount} Year{yearCount > 1 ? 's' : ''} Subscription
                            </p>
                  ...
                  <p className="mt-1 text-xs text-gray-600">
                    Total: €{(() => {
                      ...
                      if (selectedPeriod.discount_type === 'percent') {
                        discountedPrice = totalPrice - (totalPrice * selectedPeriod.amount / 100);
